    ...
    <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-2xs space-y-4 select-none font-sans">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell className="w-4 h-4 text-[#E76120]" />
          <h3 className="font-bold text-slate-900 text-sm">Advisories &amp; Notifications</h3>
        </div>
        <NavLink
      ...
      <div className="space-y-2.5">
        {notifications.length === 0 ? (
          ...
          notifications.map((notif) => (
            <div
              key={notif.id}
              className={`p-3 rounded-xl border flex items-start gap-3 transition-colors ${getBorderColor(
                notif.severity
              )}`}
            >
