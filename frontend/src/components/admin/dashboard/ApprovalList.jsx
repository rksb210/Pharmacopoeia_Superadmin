...
import { CheckCircle, XCircle, Eye, GitPullRequest, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-2xs space-y-4 select-none font-sans">
      ...
      <div className="space-y-2.5">
        {items.length === 0 ? (
          <div className="p-8 text-center bg-slate-50 border border-slate-100 rounded-xl text-slate-400">
          ...
          items.map((item, idx) => (
            <div
              key={item.id || idx}
              className="p-3.5 bg-slate-50/70 border border-slate-100 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-slate-200 transition-colors"
            >
