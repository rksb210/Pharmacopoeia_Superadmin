    ...
    <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-2xs space-y-4 select-none font-sans">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-xs">Administrative and content actions</p>
        </div>
        <NavLink
          to="/admin/audit-logs"
      ...
      <div className="space-y-3">
        {activities.length === 0 ? (
          ...
          activities.map((act) => (
            <div
              key={act.id}
              className="p-3 bg-slate-50/70 border border-slate-100 rounded-xl flex items-start gap-3 hover:border-slate-200 transition-colors"
            >
